...
public class ordenacaoparcialselecao {
    public static void main(String args[]){
        ArrayList<Personagem> personagens=Personagem.ler();
        for(int i=0; i<(10);i++){
            int menor=i;
            for(int j=i+1;j<personagens.size();j++){
                if(personagens.get(j).getNome().compareTo(personagens.get(menor).getNome())<0){
                    menor=j;
                }
            }
            Personagem tmp = personagens.get(i);
            personagens.set(i, personagens.get(menor));
            personagens.set(menor, tmp);
         }
