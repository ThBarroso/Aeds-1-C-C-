...
class Personagem{
    ...
    public Personagem(String id2, String nome2, String nomeAlternativo, String casa2, String ancestral2,
            String especie2, String patronus2, Boolean staff2, Boolean estudante2, String nomeator2, Boolean vivo2,
            ...
            try (BufferedReader br = new BufferedReader(new FileReader("/tmp/characters.csv"))) {
                ...
                while ((linha = br.readLine()) != null) {
                    ...
                    if (codigos.contains(campos[0])) {
                        ...
                        Boolean Estudante=campos[8].trim().equalsIgnoreCase("VERDADEIRO");;
...
public class countingsort {
    public static void main(String args[]){
     ...
     int maior = getMaior(personagens);
        int[] count = new int[maior + 1];
        ArrayList<Personagem> ordenado = new ArrayList<>(personagens.size());
        ...
        for (int i = personagens.size() - 1; i >= 0; i--) {
            ...
            ordenado.set(count[p.getAnonascimento()] - 1, p);
            count[p.getAnonascimento()]--;
        }

        for (int i = 0; i < personagens.size(); i++) {
            personagens.set(i, ordenado.get(i));
        }
    ...
    public static int getMaior(ArrayList<Personagem> personagens) {
        int maior = personagens.get(0).getAnonascimento();
        int contadorindex=0,indexqueestamaior=0;
        for (Personagem p : personagens) {
            contadorindex++;
            if (maior < p.getAnonascimento()) {
                maior = p.getAnonascimento();
                indexqueestamaior=contadorindex;
            }
            else if(maior==p.getAnonascimento()){
                if(personagens.get(indexqueestamaior).getNome().compareTo(p.getNome())>0){
                    maior=maior;
                }
                else{
                    maior=p.getAnonascimento();
