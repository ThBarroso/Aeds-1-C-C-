...
void imprimir(Personagem *p) {
    if (p) {
        // Preparando o campo nome_alternativo para exibição correta
        char nome_alternativo_formatado[1024] = "{}"; // Default vazio
        if (p->nome_alternativo && strlen(p->nome_alternativo) > 0) {
            snprintf(nome_alternativo_formatado, sizeof(nome_alternativo_formatado), "{%s}", p->nome_alternativo);
        }
...
void ordenacaoporselecao(Personagem **nomesusuario, int contadornomeusuario) {
    for (int i = 0; i < contadornomeusuario - 1; i++) {
        int menor = i;
        for (int j = i + 1; j < contadornomeusuario; j++) {
            if (strcmp(nomesusuario[menor]->nome, nomesusuario[j]->nome) > 0) {
                menor = j;
            }
        }
        Personagem *temp = nomesusuario[menor];
        nomesusuario[menor] = nomesusuario[i];
        nomesusuario[i] = temp;
    }
}
...
int main() {
    ...
    ordenacaoporselecao(nomesusuario,contadornomeusuario);
