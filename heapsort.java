            ...
            try (BufferedReader br = new BufferedReader(new FileReader("/tmp/characters.csv"))) {
                ...
                while ((linha = br.readLine()) != null) {
                    ...
                    if (codigos.contains(campos[0])) {
                        ...
                        Boolean Staff= Boolean.parseBoolean(campos[7]);
                        String Estudante=campos[8];
                        String Nomeator=campos[9];
                        Boolean Vivo= Boolean.parseBoolean(campos[10]);
...
class heap{
    public void sort(ArrayList<Personagem> listapersonagens){
        int tamanholista=listapersonagens.size();
        //construção do heap
        for(int tamheap=2;tamheap<=tamanholista;tamheap++){
            construir(listapersonagens,tamheap);
        }

        //ordenação do heap construido
        int tamheap=tamanholista;
        while(tamheap>1){
            swap(listapersonagens,0,tamheap-1);
            tamheap--;
            reconstruir(listapersonagens,tamheap,0);
        }
    }
    public void construir(ArrayList<Personagem> listapersonagens,int tamheap){
        int indicePai=(tamheap-1)/2;
        for(int i=indicePai;i>=0;i--){
            reconstruir(listapersonagens,tamheap,i);
        }
    }

    public void reconstruir(ArrayList<Personagem> listapersonagens,int tamheap,int indicepai){
        int maior= indicepai;
        int esquerda =2*indicepai+1;
        int direita = 2*indicepai+2;

        if(esquerda<tamheap && listapersonagens.get(esquerda).getCorcabelo().compareTo(listapersonagens.get(maior).getCorcabelo())>0){
            maior=esquerda;
        }
        if(direita<tamheap && listapersonagens.get(direita).getCorcabelo().compareTo(listapersonagens.get(maior).getCorcabelo())>0){
            maior=direita;
        }
        if(maior!=indicepai){
            if(!listapersonagens.get(indicepai).getCorcabelo().equals(listapersonagens.get(maior).getCorcabelo())){
            swap(listapersonagens,indicepai,maior);
            reconstruir(listapersonagens, tamheap, maior);
            }
        }

    }
    public void swap(ArrayList<Personagem> listapersonagens,int indicepai,int maior){
        Personagem temp = listapersonagens.get(indicepai);
        listapersonagens.set(indicepai,listapersonagens.get(maior));
        listapersonagens.set(maior,temp);
    }
}
...
public class heapsort {
    public static void main(String args[]){
        ...
        heap novoheap = new heap();
        novoheap.sort(personagens);
